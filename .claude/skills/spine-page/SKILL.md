...
Useful attribute properties, all optional: `Lifetime` (`Transient` default; `Singleton` keeps state — tabs default to `Singleton`), `IsHeaderBarVisible`, `IsBackButtonVisible`, `TitleAlignment`, `SafeAreaEdges` (exclude an edge to draw behind it, then offset with `ViewModelBase.SafeAreaInsets`), `ScrollInset` (edges on which the page's first `ScrollView`/`CollectionView` takes that inset as a native content inset, so a list can scroll under a bar and still reach its last row; the same thing per view is `SafeArea.ScrollInset="Bottom"`). For a page on a photo or hero: `HeaderBar = HeaderBarMode.Overlay` (header floats over content that starts at the top), `HeaderBarForeground = "#FFFFFF"` (title and icons in a fixed colour) and `StatusBarStyle = StatusBarStyle.LightContent` (iOS needs `UIViewControllerBasedStatusBarAppearance` false in Info.plist); give the list `SafeArea.ScrollInset="Top"`. For an iOS-style large title: `LargeTitle = true`, put `<HeaderBarLargeTitle />` first in the page's `ScrollView`/`CollectionView` (e.g. `CollectionView.Header`) — it shows the page title in the platform's large-title size and is what Spine measures (custom titles: `HeaderBarConstants.LargeTitle…` plus `HeaderBar.CollapseDistance`; the whole bar's height is `HeaderBarConstants.BarHeight`, 54 on iOS 26 with the 44-point `Height` item row at its top); Spine gives that scroll view its top inset, fades the bar's title and background in as the label scrolls under the bar, and publishes `ViewModelBase.HeaderBarCollapseProgress`. `HeaderBar.ScrollSource="{x:Reference List}"` on the page picks the scroll view (default: the first), `HeaderBarBackground = ScrollEdge | ScrollEdgeSoft | ScrollEdgeHard | Solid | Clear` (default `Auto`: `Clear` under `Overlay`; on iOS 26 `ScrollEdge` — the system scroll edge effect over the status bar and the whole bar, content under the bar, top inset added — for a region/tab page whose `ScrollView`/`CollectionView` fills it from the top; `Solid` otherwise) decides what is behind the bar while content scrolls under it. `ScrollEdge` takes UIKit's automatic style (soft on iPhone), `ScrollEdgeSoft`/`ScrollEdgeHard` pick UIKit's soft fade or hard frosted band with a hairline. Android/Windows show a fading band (soft) or a nearly opaque band with a hairline (hard) for an explicit scroll edge value; Reduce Transparency gives `Solid`. Set `HeaderBarBackground` (or `HeaderBarMode`) on the view model to change it while the page is shown — Spine re-resolves and re-lays out the page; layout (`HeaderBar`), `LargeTitle` and `HeaderBarBackground` combine freely, each also an app-wide default in `options.RegionDefaults`/`TabDefaults`/`SheetDefaults`.
