...
public sealed record LocalNotification
{
    ...
    /// <see cref="IPushNotificationHandler.OnOpenedAsync"/> navigates the same way whichever half sent it.
    /// </summary>
    public string? Route { get; init; }

    /// <summary>
    /// The channel from <see cref="SpinePushNotificationsOptions.AddChannel"/> to post it in, and the thread id on
    /// Apple. The app's first channel when <see langword="null"/>.
    /// </summary>
    public string? Channel { get; init; }

    /// <summary>The buttons to show, by the id given to <see cref="SpinePushNotificationsOptions.AddCategory"/>.</summary>
    ...
    /// Android the channel decides — see <see cref="SpinePushNotificationsOptions.AddChannel"/>.
    /// </summary>
...
/// <see cref="IPushNotificationService.RequestPermissionAsync"/>; the channels are the ones
/// <see cref="SpinePushNotificationsOptions.AddChannel"/> created; and an opened notification reaches the app's
/// <see cref="IPushNotificationHandler"/> with <see cref="PushMessage.IsLocal"/> set, so a handler that does not
...
/// because the backend already pushes it — <see cref="IPushNotificationService.IsRegistered"/> is the signal for
