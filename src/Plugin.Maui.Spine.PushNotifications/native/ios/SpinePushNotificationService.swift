...
@objc(SpinePushNotificationService)
final class SpinePushNotificationService: UNNotificationServiceExtension {
    ...
    override func didReceive(
        ...
        download = URLSession.shared.downloadTask(with: url) { [weak self] location, response, error in
            ...
            if let location, let attachment = Self.attachment(at: location, response: response, url: url) {
                ...
                NSLog("SpinePushNotificationService: no picture from %@: %@",
    ...
    override func serviceExtensionTimeWillExpire() {
        download?.cancel()
        NSLog("SpinePushNotificationService: out of time fetching the picture; delivering the text")
        deliver()
    }
    ...
    private static func attachment(at location: URL, response: URLResponse?, url: URL) -> UNNotificationAttachment? {
        ...
        do {
            ...
            NSLog("SpinePushNotificationService: could not attach %@: %@", url.absoluteString, error.localizedDescription)
            return nil
        }
    }
