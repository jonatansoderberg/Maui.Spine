...
# Plugin.Maui.Spine.PushNotifications.targets when SpinePushNotificationsImages is on, with every input as an argument.
set -euo pipefail

OUT=""; SOURCES=""; SDK="iphonesimulator"; ARCH="arm64"; MIN_OS="15.0"; CONFIG="Debug"
BUNDLE_ID=""; NAME="SpinePushNotificationService"; DISPLAY_NAME=""; PROVISION=""; REQUIRE_PROVISION="false"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "spine-push-notifications-build.sh: unknown argument $1" >&2; exit 2;;
  esac
done

[[ -n "$OUT" && -n "$SOURCES" && -n "$BUNDLE_ID" ]] || { echo "spine-push-notifications-build.sh: --out, --sources and --bundle-id are required" >&2; exit 2; }
...
case "$SDK" in
  ...
  *) echo "spine-push-notifications-build.sh: unsupported sdk $SDK" >&2; exit 2;;
esac
...
# The principal class is named with @objc in the Swift source, so it is not module-qualified here.
{
  ...
  cat <<PLIST
	...
	<dict>
		...
		<key>NSExtensionPrincipalClass</key><string>SpinePushNotificationService</string>
	</dict>
</dict>
</plist>
PLIST
...
xcrun -sdk "$SDK" swiftc \
  ...
  -o "$APPEX/$NAME" \
  "$SOURCES/SpinePushNotificationService.swift"
...
if [[ "$SDK" == "iphoneos" ]]; then
  if PROFILE=$(find_profile); then
    ...
    echo "spine-push-notifications-build.sh: embedded $(basename "$PROFILE") for $BUNDLE_ID"
  elif [[ "$REQUIRE_PROVISION" == "true" ]]; then
    {
      echo "spine-push-notifications-build.sh: no provisioning profile for the Notification Service Extension."
      echo ""
      echo "  SpinePushNotificationsImages=true adds an extension that fetches pictures for pushed notifications."
      ...
      echo ""
      if [[ -n "$PROVISION" ]]; then
        echo "  SpinePushNotificationsImagesCodesignProvision is '$PROVISION'; no installed profile has that name"
        echo "  and that App ID. Leave it empty to take whichever installed profile matches."
      else
        echo "  Install it, then build again. Name a specific one with SpinePushNotificationsImagesCodesignProvision."
      fi
      echo "  Set SpinePushNotificationsImages=false to build without pictures in pushed notifications on iOS."
    } >&2
    exit 3
  fi
fi
...
echo "spine-push-notifications-build.sh: built $APPEX for $TARGET"
