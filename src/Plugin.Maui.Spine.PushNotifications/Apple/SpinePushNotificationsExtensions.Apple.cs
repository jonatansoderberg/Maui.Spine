...
public static partial class SpinePushNotificationsExtensions
{
    ...
    private static async Task Start(ApplePushPlatform platform, SpinePushNotificationsOptions options, UIApplication application)
    {
        ...
        // there follows the signing, see Plugin.Maui.Spine.PushNotifications.targets.
    ...
    private static async Task Resume(ApplePushPlatform platform, SpinePushNotificationsOptions options)
    {
        await platform.RefreshStatusAsync();
    ...
    /// </summary>
    private static void RegisterCategories(SpinePushNotificationsOptions options)
    {
    ...
    {
        if (SpinePushNotifications.NotInstalled.Count == 0) return;

        Services().GetRequiredService<ILogger<IPushNotificationService>>().LogWarning(
            "Spine.PushNotifications: the app's AppDelegate already implements {Selectors}, so Spine did not add them. " +
            "Call the matching SpinePushNotifications.Forward member from each, or push will not reach the handler.",
            string.Join(", ", SpinePushNotifications.NotInstalled));
    }

    private sealed class NotificationDelegate(SpinePushNotificationsOptions options) : UNUserNotificationCenterDelegate
    {
