...
/// </summary>
/// <remarks>
/// The foreground presentation is not decided here. Spine.PushNotifications owns
/// <c>UNUserNotificationCenter.Current.Delegate</c> and the system hands it local notifications too,
/// so the app's <see cref="IPushNotificationHandler"/> is asked what to show — the same answer it gives for a
/// push.
/// </remarks>
internal sealed class AppleLocalNotifications : ILocalNotificationService
{
    ...
    public async Task SyncAsync(IEnumerable<LocalNotification> plan, CancellationToken cancellationToken = default)
    {
        ...
        {
            Logger?.LogWarning(
                "Spine.PushNotifications: {Count} local notifications planned, but iOS keeps {Limit}. The {Limit} nearest were scheduled.",
        ...
        foreach (var notification in due)
        {
            ...
            {
                Logger?.LogWarning("Spine.PushNotifications: scheduling {Id} failed: {Error}", notification.Id, e.Error.LocalizedDescription);
            }
        }
    }
    ...
    private static UNNotificationAttachment? Attachment(LocalNotification notification)
    {
        ...
        {
            Logger?.LogWarning("Spine.PushNotifications: no picture for {Id}: '{Image}' is not a file on the device.", notification.Id, image);
            return null;
        }
        ...
        if (error is not null)
            Logger?.LogWarning("Spine.PushNotifications: no picture for {Id}: {Error}", notification.Id, error.LocalizedDescription);
