...
/// <summary>The Apple half of <see cref="IPushNotificationService"/>.</summary>
internal sealed class ApplePushPlatform : IPushPlatform
{
    ...
    private readonly SpinePushNotificationsOptions _options;

    internal ApplePushPlatform(SpinePushNotificationsOptions options)
    {
        _options = options;
        _current = this;
    }
    ...
    public async Task<PushStatus> RequestPermissionAsync(PushPermission permission, CancellationToken cancellationToken)
    {
        ...
        if (error is not null)
            Logger?.LogWarning("Spine.PushNotifications: requesting authorization failed: {Error}", error.LocalizedDescription);
    ...
    internal static void DidFailToRegister(NSError? error) =>
        Logger?.LogWarning("Spine.PushNotifications: APNs registration failed: {Error}", error?.LocalizedDescription ?? "unknown");
        ...
        NSDictionary userInfo, Action<UIBackgroundFetchResult> completionHandler, bool isColdStart)
    {
        ...
        try
        {
            ...
            await SpinePushNotificationsExtensions.DeliverAsync(services, message, context);
            completionHandler(UIBackgroundFetchResult.NewData);
        }
        catch (Exception e)
        {
            Logger?.LogError(e, "Spine.PushNotifications: handling a remote notification failed.");
    ...
    /// </summary>
    /// <summary>
    /// Whether the build set the app up for remote push. The targets write <c>SpinePushNotificationsEnvironment</c>
    /// into Info.plist exactly when they add the push entitlement, so it is absent with
    /// <c>SpinePushNotificationsRemote=false</c> — and on Mac Catalyst without a provisioning profile, where the
    /// entitlement cannot be used and an app signed with it would not launch.
    /// </summary>
    internal static bool IsRemoteConfigured { get; } =
        NSBundle.MainBundle.ObjectForInfoDictionary("SpinePushNotificationsEnvironment") is not null;

    private static ApnsEnvironment ReadEnvironment() =>
        NSBundle.MainBundle.ObjectForInfoDictionary("SpinePushNotificationsEnvironment")?.ToString() switch
        {
