...
internal static class AndroidNotifications
{
        ...
        foreach (var channel in _channels)
        {
            ...
            if (channel.Sound is { Length: > 0 } sound)
            {
                ...
                if (context.Resources!.GetIdentifier(sound, "raw", context.PackageName) == 0)
                    Android.Util.Log.Warn("Spine.PushNotifications", $"Channel '{channel.Id}': no sound named '{sound}' in Resources/Raw; it gets the system sound.");
                else
                    builder.SetSound(
    ...
    internal static void Show(Context context, PushMessage message, PushPresentation presentation, Bitmap? picture = null)
    {
        ...
        if (message.Category is { Length: > 0 } category)
        {
            ...
            {
                Android.Util.Log.Warn("Spine.PushNotifications", $"The notification names category '{category}', which the app never declared with AddCategory; it has no buttons.");
            }
        }
    ...
    internal static async Task<Bitmap?> PictureAsync(PushMessage message, CancellationToken cancellationToken)
    {
        ...
        {
            Android.Util.Log.Warn("Spine.PushNotifications", $"No picture: {why} The notification is shown without it.");
            return null;
        }
    }
