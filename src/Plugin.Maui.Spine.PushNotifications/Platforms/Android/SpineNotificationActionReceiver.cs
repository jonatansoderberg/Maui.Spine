...
/// <see cref="IPushNotificationHandler.OnActionAsync"/>. Android starts the app's process for the broadcast if it
...
internal sealed class SpineNotificationActionReceiver : BroadcastReceiver
{
    ...
    public override void OnReceive(Context? context, Intent? intent)
    {
        ...
        if (intent.GetStringExtra(AndroidNotifications.ActionExtra) is not { } action) return;
        if (AndroidNotifications.Read(intent) is not { } message) return;

        var text = AndroidX.Core.App.RemoteInput.GetResultsFromIntent(intent)?.GetCharSequence(AndroidNotifications.ReplyKey)?.ToString();
        var notificationId = intent.GetIntExtra(AndroidNotifications.NotificationIdExtra, 0);
        ...
        Task.Run(async () =>
        {
            try
            {
                await SpinePushNotificationsExtensions.ActionAsync(services, message, action, text);
            }
            finally
            {
