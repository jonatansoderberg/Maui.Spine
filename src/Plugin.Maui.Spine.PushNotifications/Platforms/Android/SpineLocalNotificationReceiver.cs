    ...
    public override void OnReceive(Context? context, Intent? intent)
    {
        ...
        Task.Run(async () =>
        {
            try
            {
                ...
                // behave alike.
                if (IPlatformApplication.Current?.Services is { } services && SpinePushNotificationsLifecycle.IsForeground)
                {
                    using var deadline = new CancellationTokenSource(TimeSpan.FromSeconds(10));
                    var reception = new PushContext(true, false, DateTimeOffset.UtcNow, deadline.Token);
                    presentation = await SpinePushNotificationsExtensions.DeliverAsync(services, message, reception);

                    if (presentation == PushPresentation.None) return;
                }

                var picture = await AndroidNotifications.PictureAsync(message, CancellationToken.None);
                AndroidNotifications.Show(context, message, presentation, picture);
            }
