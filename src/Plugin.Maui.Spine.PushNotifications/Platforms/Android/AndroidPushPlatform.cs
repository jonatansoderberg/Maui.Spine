...
/// <summary>The Android half of <see cref="IPushNotificationService"/>.</summary>
internal sealed class AndroidPushPlatform : IPushPlatform
{
    ...
    private static async Task<bool> TopicAsync(string topic, bool subscribe)
    {
        try
        {
            ...
            Logger?.LogInformation("Spine.PushNotifications: {Action} FCM topic {Topic}.", subscribe ? "subscribed to" : "unsubscribed from", topic);
            return true;
        }
        catch (Exception e)
        {
            Logger?.LogWarning(e, "Spine.PushNotifications: could not {Action} FCM topic {Topic}.", subscribe ? "subscribe to" : "unsubscribe from", topic);
            return false;
        }
    }
...
#pragma warning restore CS0618
        ...
        {
            Logger?.LogWarning(e, "Spine.PushNotifications: could not read the FCM registration token.");
        }
    }
