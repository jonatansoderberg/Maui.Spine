...
{
    /// <summary>Only when the app calls <see cref="IPushNotificationService.RequestPermissionAsync"/>. The default.</summary>
    WhenAsked,
...
public sealed record PushAction(string Id, string Title)
{
    ...
    /// <see cref="IPushNotificationHandler.OnOpenedAsync"/>, where navigating is safe. When it does not, it reaches
    /// <see cref="IPushNotificationHandler.OnActionAsync"/> in the background — for "mark as read" or "sign me up",
    ...
    /// <see cref="IPushNotificationHandler.OnActionAsync"/> as its text. A reply never opens the app, whatever
...
public sealed class SpinePushNotificationsOptions
{
    ...
    /// the prefix given to <c>MapSpinePushNotifications</c> on the server.
    /// </summary>
    ...
    /// <summary>Registers the app's <see cref="IPushNotificationHandler"/>.</summary>
    /// <typeparam name="THandler">The handler to resolve through DI for every message.</typeparam>
    /// <returns>The same options, for chaining.</returns>
    public SpinePushNotificationsOptions UseHandler<THandler>() where THandler : class, IPushNotificationHandler
    {
    ...
    /// </param>
    /// <returns>The same options, for chaining.</returns>
    public SpinePushNotificationsOptions AddChannel(
        string id, string name, PushChannelImportance importance = PushChannelImportance.Default, string? sound = null)
    {
    ...
    /// <param name="actions">The buttons, in the order they are shown.</param>
    /// <returns>The same options, for chaining.</returns>
    public SpinePushNotificationsOptions AddCategory(string id, params PushAction[] actions)
    {
    ...
    {
        if (Backend is null)
            throw new InvalidOperationException("UseSpinePushNotifications: Backend is required before the app can register.");
