    ...
    internal void Validate()
    {
        ...
        static void Require(string? value, string name)
        {
            if (string.IsNullOrWhiteSpace(value))
                throw new InvalidOperationException($"AddSpinePushNotifications: Apple.{name} is required when Apple(...) is configured.");
        }
    }
}
    ...
    internal void Validate()
    {
        if (string.IsNullOrWhiteSpace(ServiceAccountJson))
            throw new InvalidOperationException(
                $"AddSpinePushNotifications: Android.{nameof(ServiceAccountJson)} is required when Android(...) is configured.");
    }
}
...
public sealed class WindowsPushOptions
{
    ...
    internal void Validate()
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(value))
                throw new InvalidOperationException($"AddSpinePushNotifications: Windows.{name} is required when Windows(...) is configured.");
        }
    }
}

/// <summary>How Spine.PushNotifications behaves on this server.</summary>
public sealed class SpinePushNotificationsOptions
{
    ...
    public SpinePushNotificationsOptions Apple(Action<ApplePushOptions> configure)
    {
    ...
    /// <returns>The same options, for chaining.</returns>
    public SpinePushNotificationsOptions Android(Action<AndroidPushOptions> configure)
    {
        ArgumentNullException.ThrowIfNull(configure);
    ...
    /// <returns>The same options, for chaining.</returns>
    public SpinePushNotificationsOptions Windows(Action<WindowsPushOptions> configure)
    {
        ArgumentNullException.ThrowIfNull(configure);
    ...
    /// <returns>The same options, for chaining.</returns>
    public SpinePushNotificationsOptions UseInMemoryStore()
    {
    ...
    /// <returns>The same options, for chaining.</returns>
    public SpinePushNotificationsOptions UseAzureTableStore(string connectionString, string prefix = "SpinePush")
    {
    ...
    /// <returns>The same options, for chaining.</returns>
    public SpinePushNotificationsOptions UseStore(Func<IServiceProvider, IPushInstallationStore> factory)
    {
    ...
    internal void Validate()
    {
        ...
        if (StoreFactory is null)
            throw new InvalidOperationException("AddSpinePushNotifications: pick a register with UseInMemoryStore() or UseStore(...).");
    }
