...
- **`WidgetColor.From` har två överlagringar, en per skikt.** `Common` äger `From(System.Drawing.Color)` på BCL-typen, så en server som bygger träd utan MAUI har en färgingång utöver `FromHex` — det behövs i #176. `Widgets` äger `From(Microsoft.Maui.Graphics.Color)` som extension-medlem. De kan inte bli tvetydiga: det finns ingen konvertering mellan de två färgtyperna, verifierat genom reflektion över `Microsoft.Maui.Graphics` (enda konverteringsoperatorn på `Color` är `Vector4 → Color`, och inget i assemblyn nämner `System.Drawing`). Varje anrop löser alltså upp till exakt en av dem, och anropsställena i Orientera är orörda.
- **Alternativet att referera `Microsoft.Maui.Graphics` från `Common` förkastades.** Det fungerar — testat hela vägen, inklusive att `Orientera.Backend` kompilerar mot det utan MAUI-workload — och hade gjort `From(Color)` till en vanlig medlem. Men `Plugin.Maui.Spine.Server` i #176 ärver allt `Common` beror på, och ett serverpaket med ett MAUI-namn i beroendeträdet är svårare att motivera än 17 rader i Widgets. `Common` har noll paketreferenser.
