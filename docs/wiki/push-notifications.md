...
`samples/MauiSpinePushNotificationsSampleApp` is the shortest path to seeing this work. It shows the whole client
API on six pages — status and tokens, tags, a form that asks the server to send, a log of everything
the handler received, a Live Activity, and local notifications — and `samples/MauiSpinePushNotificationsSampleApp.Server` is a Minimal
API on `Plugin.Maui.Spine.Server` with an in-memory register.

```bash
cd samples/MauiSpinePushNotificationsSampleApp.Server && dotnet run     # then run the app
...
{
    SpinePushNotifications.Install();                                  // ← before UIApplication.Main
...
selector, and lists it in `SpinePushNotifications.NotInstalled`. Call the matching forwarder from your own
implementation:
...
{
    SpinePushNotifications.Forward.DidReceive(userInfo, completionHandler);
}
```
...
| `NoToken` | The platform has not issued a token — read `IPushNotificationService.Token` to see. |
...
platforms, so `IPushNotificationService.RequestPermissionAsync` is the only place the user is asked. The channels
are the ones `AddChannel` created. And an opened local notification reaches the same
`IPushNotificationHandler.OnOpenedAsync`, carrying the same `spine.route`, so navigation does not need to know
which half sent it:
...
notifications the backend also sends — but `IPushNotificationService.IsRegistered` is the signal to make it with:
...
```xml
<SpinePushNotificationsRemote>false</SpinePushNotificationsRemote>
```
...
with the package, so Android's `SupportedOSPlatformVersion` floor of 23 stays. `SpinePushNotificationsEnabled=false`
...
under `Spine.PushNotifications` on Android, `SpinePushNotificationService` in the device log on iOS. The server refuses
...
```xml
<SpinePushNotificationsImages>true</SpinePushNotificationsImages>
```

It is off by default because it is a bundle of its own: device builds need an App ID
`<ApplicationId>.SpinePushNotificationService` and a development profile for it, exactly like the widget
extension, and the build stops with the details when there is none. Name a specific profile with
`SpinePushNotificationsImagesCodesignProvision`. Simulator builds need nothing. The server sets
...
shows `SpinePushNotificationService` handing back the notification with its attachment.
