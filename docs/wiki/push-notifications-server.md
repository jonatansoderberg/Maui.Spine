...
The app half is [`Plugin.Maui.Spine.PushNotifications`](push-notifications.md). The two share `Plugin.Maui.Spine.Common`, which is where
...
```csharp
services.AddSpinePushNotifications(o =>
{
    o.Apple(a =>
    {
...
`AddSpinePushNotifications` validates as it goes: a missing team id or a server with no register throws with the
...
- **Android follows an FCM topic** named after the channel (`LiveActivityChannels.Topic`).
  Spine.PushNotifications subscribes while an activity on the channel runs and unsubscribes when the last one
...
```csharp
app.MapSpinePushNotifications("/push");
```
...
public Task<IResult> Push([HttpTrigger(AuthorizationLevel.Anonymous, "put", "delete",
    ...
    => SpinePushNotificationsEndpoints.HandleAsync(request, ct);
```
